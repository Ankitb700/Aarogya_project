...
class Model:
    ...
    def __enter__(self):
        ...
        reuse_pool = (hasattr(self, 'face_detection_pool') and self.face_detection_pool is not None
                      and not self.face_detection_pool._shutdown)
        if not reuse_pool:
            self.face_detection_pool = ThreadPoolExecutor(max_workers=self.face_detection_threads)
        reuse_resample = (hasattr(self, 'face_resampling_pool') and self.face_resampling_pool is not None
                          and not self.face_resampling_pool._shutdown)
        if not reuse_resample:
    ...
    def wait_completion(self):
        if self.run is None:
            return
        import threading
        if threading.current_thread() is not self.run:
            self.run.join()
